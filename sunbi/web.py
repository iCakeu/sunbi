...
class API:
    def __init__(self,地址,抬头='',数据包=''):
        ...
        if 数据包 == '':
            ...
            self.__result = json.loads(requests.post(地址,抬头,json.dumps(数据包)).text)
...
class 网页模拟:
    ...
    def 访问网址(self,网址):
        self.__driver.get(网址)
        
    def 点击元素(self,xpaths):
        '''字符串列表则循环点击'''
        if isinstance(xpaths,str):
            self.__driver.find_element_by_xpath(xpaths).click()
        else:
            for xpath in xpaths:
                self.__driver.find_element_by_xpath(xpath).click()
    
    def 输入内容(self,xpaths,文本):
        if isinstance(xpaths,str) or isinstance(文本,str):
            self.__driver.find_element_by_xpath(xpaths).send_keys(文本)
        else:
            for (xpath,text) in zip(xpaths,文本):
                self.__driver.find_element_by_xpath(xpath).send_keys(text)
    
    def 按回车(self,xpaths):
        if isinstance(xpaths,str):
            self.__driver.find_element_by_xpath(xpaths).send_keys(KEYS.ENTER)
        else:
            for xpath in xpaths:
                self.__driver.find_element_by_xpath(xpath).send_keys(KEYS.ENTER)

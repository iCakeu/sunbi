...
class 表格:
    ...
    def 生成图片(self,对象,文件夹,格式='jpg'):
        '''
        从Excel中获取图片(在__init__设置表),保存为本地图片
        ...
        if isinstance(对象,(list,tuple,set)):
            ...
            self.__imgpaths.append(self._saveImg(对象))

        return self.__imgpaths

    def _saveImg(self,对象):
        '''
        无论是对象名还是索引号 方法都是一样的
        '''
        self.__sht.shapes(对象).copy() 
        img = ImageGrab.grabclipboard()
        self.__imgpath = os.path.join(self.__folder,'{filename}.{format}'.format(filename=对象,format=self.__format))
        img.save(self.__imgpath)
        print('图片已保存:{path}'.format(self.__imgpath))
        time.sleep(1)
        return self.__imgpath

    def 获取单元格文本(self,行列=((行,列),)):
        textList = []
        for (x,y) in 行列:
            textList.append(self.__sht.Cells(x,y).value)
        return '\n'.join(textList)
    ...
    def 导出(self,文件路径,嵌套对象,标题=[]):
        '''
        嵌套对象导出为工作簿 常见从数据库导出到Excel
        '''
        if len(标题) > 0:
